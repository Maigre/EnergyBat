...
class awFileFontDriver extends awFontDriver {
	...
	private function gdString(awGDDriver $driver, awText $text, awPoint $point, $width = NULL) {
		...
		$box = imagettfbbox($font->size, $text->getAngle(), ARTICHOW.'/font/'.$font->name.'.'.$font->getExtension(), $text->getText());
		$textHeight = - $box[5];

		$box = imagettfbbox($font->size, 90, ARTICHOW.'/font/'.$font->name.'.'.$font->getExtension(), $text->getText());
		...
		imagettftext(
			...
			$rgb,
			ARTICHOW.'/font/'.$font->name.'.'.$font->getExtension(),
			$textString
		);
	}
	...
	private function getGDTextWidth(awText $text) {
		...
		//echo $font->name.'.'.$font->getExtension(); die;
